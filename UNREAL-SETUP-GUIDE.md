...
| `BoneAngles` | MoCapBoneAngles | Type `MoCapBoneAngles` in the type dropdown. Unreal strips the `F` prefix from struct names, so the C++ type `FMoCapBoneAngles` appears as `MoCap Bone Angles` or `MoCapBoneAngles` | (leave default) |

The `MoCapWebSocket` and `MoCapBoneAngles` types appear in the dropdown because you compiled the C++ class in step 5. If they do not appear, go back and confirm the compile succeeded.
...
5. Now read the pose data. Drag `MoCapSocket` into the graph again (select **Get** this time). Drag off it and type `Is Idle`. Select **Get Is Idle**. Note: Unreal strips the `b` prefix from boolean properties, so the C++ variable `bIsIdle` appears as `Is Idle` in Blueprints. Wire this into a **Set** node for your local `bIsIdle` variable.
6. Drag off `MoCapSocket` again and type `Bone Angles`. Select **Get Bone Angles**. Wire this into a **Set** node for your local `BoneAngles` variable.
...
Event Blueprint Update Animation
  ...
  > Set bIsIdle (from MoCapSocket -> Is Idle)
  > Set BoneAngles (from MoCapSocket -> Bone Angles)
```
...
To access the bone angles, drag your `BoneAngles` variable into the graph (Get), then drag off it and select **Break MoCap Bone Angles** (Unreal may show it as `Break MoCapBoneAngles`). This splits the struct into individual float pins that you can wire directly into Make Rotator nodes.
