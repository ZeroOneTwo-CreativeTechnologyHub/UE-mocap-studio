...
| Property | Blueprint Name | Type | What It Contains |
|---|---|---|---|
| `bIsIdle` | **Is Idle** | Boolean | True when no person is detected. Use this to switch states. |
| `bIsConnected` | **Is Connected** | Boolean | True when connected to the bridge. |
| `Landmarks` | **Landmarks** | Array of FMoCapLandmark | All 33 tracked points with raw coords, visibility, and world-space positions. |
| `BoneAngles` | **Bone Angles** | FMoCapBoneAngles | Pre-computed angles: spine tilt, head tilt, elbow, knee, shoulder, hip. |
| `CaptureFPS` | **Capture FPS** | Integer | Frame rate reported by the browser app. |

Unreal automatically reformats C++ property names for the Blueprint editor: the `b` prefix is stripped from booleans, and camelCase becomes separate words. The "Blueprint Name" column shows what you will actually see in the editor.
...
### 6.1 Add a Variable for the MoCap Actor

In the **My Blueprint** panel, add one variable:
...
This type appears in the dropdown because you compiled the C++ class in step 5. If it does not appear, go back and confirm the compile succeeded.

You do not need variables for `IsIdle` or `BoneAngles`. The Animation Blueprint reads those directly from the `MoCapSocket` reference wherever it needs them.

### 6.2 Find the MoCap Actor Every Frame
...
4. Drag your `MoCapSocket` variable from the **My Blueprint** panel into the graph and select **Set**. Connect the blue output pin from the Get node into the blue input pin on the Set node. Connect the white execution pin from **Get All Actors of Class** to the **Set MoCapSocket** node.

That is the entire Event Graph. It looks like this:
...
Every tick, this finds the MoCap actor and stores a reference to it. The state machine and Modify Bone nodes in the AnimGraph will read `Is Idle` and `Bone Angles` directly from this reference. You will set that up in the next steps.
...
3. Double-click the **Idle** state to open it. You will see a **Result** node with an animation pose pin.
4. You need to add an animation player. There are two ways:

   **Option A (easiest):** Open the **Content Browser** at the bottom of the editor. Find an idle animation asset, then **drag it directly** from the Content Browser into this state graph. A sequence player node appears automatically.

   **Option B:** Right-click in the state graph, and in the search menu, type the name of your idle animation (e.g. `MM_Idle`). Select it from the results. This creates a sequence player node for that animation.

   Which animation to use:
   - For the **UE5 mannequin**: look in `Content > Characters > Mannequins > Animations` for an idle animation such as `MM_Idle` or `MF_Idle`.
   - For **Mixamo characters**: go back to mixamo.com, search for "Breathing Idle" or "Standing Idle", download as FBX, and import into your project (this time with Import Animations checked). Then use that asset.

5. Select the sequence player node you just created. In the **Details** panel on the right, find **Loop Animation** and check the box to set it to **True**.
6. Connect the output pose pin from the sequence player node to the **Result** node's input pose pin. If it connected automatically when you dragged it in, you are already done.

---

## 8. Build the MoCap State

### 8.1 Add the MoCap State

1. Go back to the state machine view.
2. Right-click and **Add State**. Name it `MoCapActive`.
3. Create transitions:
   - **Idle to MoCapActive:** Double-click the transition arrow. Drag your `MoCapSocket` variable into the graph (Get), drag off it and type `Is Idle`, select **Get Is Idle**. Add a **NOT Boolean** node after it. Connect the result to the **Can Enter Transition** pin. This means: enter MoCapActive when Is Idle is false (a person is detected).
   - **MoCapActive to Idle:** Double-click this transition arrow. Same setup but without the NOT node. Drag `MoCapSocket` in, get **Is Idle**, and connect directly to **Can Enter Transition**. This means: go back to Idle when Is Idle is true (person left the frame).
...
### 8.2 Build the MoCap Pose Logic
...
To access the bone angles, drag your `MoCapSocket` variable into the graph (Get), drag off it and type `Bone Angles`, select **Get Bone Angles**. Then drag off that and select **Break MoCap Bone Angles** (Unreal may also show it as `Break MoCapBoneAngles`). This splits the struct into individual float pins that you can wire directly into Make Rotator nodes.
